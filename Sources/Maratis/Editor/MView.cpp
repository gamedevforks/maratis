...
void MView::initPerspective(void)
{
...
void MView::rotate(float mx, float my)
{
	// inverse center
...
void MView::pan(float mx, float my)
{
...
void MView::zoom(float mz)
{
	...
	{
		float dFactor = ((m_pivot - position).getLength() * 0.01f) * (mz * 20.0f);

		MVector3 vec(0, 0, -1);
		MVector3 axis = m_camera.getRotatedVector(vec);

		m_camera.setPosition(position - (axis * dFactor));
		m_camera.updateMatrix();
	}
}

void MView::switchProjectionMode(void)
{
	MVector3 cameraAxis = m_camera.getRotatedVector(MVector3(0, 0, -1)).getNormalized();;
	MVector3 position = m_camera.getPosition();
