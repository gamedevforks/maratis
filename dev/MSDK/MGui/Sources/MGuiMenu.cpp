...
void MGuiMenu::onEvent(MWinEvent * windowEvent)
{
	...
	case MWIN_EVENT_MOUSE_MOVE:
		if(parent->isHighLight() && isMouseInside() && (! mouse->isLeftButtonPushed()) && (! mouse->isMiddleButtonPushed()) && (! mouse->isRightButtonPushed()))
		{
			setHighLight(true);
...
void MGuiMenu::rescaleWindowMenu(void)
{
	...
	if((newPos.y + newScale.y) > window->getHeight()){
		newPos.y -= ((newPos.y + newScale.y) - window->getHeight());
	}
